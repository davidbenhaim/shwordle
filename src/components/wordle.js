...
const wordBank = {};
words.forEach(word => {
  wordBank[word.toUpperCase()] = true;
});
...
const Worlde = () => {
  ...
  const handleSubmit = () => {
    ...
    if(wordBank[currentGuess.toUpperCase()] === undefined) return;
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-background text-foreground p-4">
      <h1 className="text-4xl font-bold mb-8">SHWORDLE</h1>
      <div className="grid grid-rows-6 gap-2 mb-4">
        {guesses.map((guess, i) => (
          <div key={i} className="grid grid-cols-5 gap-2">
            {Array.from(guess || '_____').map((letter, j) => (
              <div 
                key={j} 
                className={`w-12 h-12 border-2 flex items-center justify-center font-bold text-2xl
                  ${!guess ? 'border-border' :
                    letter === solution[j] ? 'bg-green-500 border-green-500 text-primary-foreground' :
                    solution.includes(letter) ? 'bg-yellow-500 border-yellow-500 text-primary-foreground' :
          ...
          <div key={i} className="flex justify-center mb-2">
            {row.map((key) => (
              <Button
                ...
                className={`mx-1 border border-white ${
                  key.length > 1 ? 'px-4' : 'w-10 h-10'
                } ${
                  usedLetters[key] === 'correct' ? 'bg-green-500 hover:bg-green-600' :
                  usedLetters[key] === 'present' ? 'bg-yellow-500 hover:bg-yellow-600' :
      ...
      {gameOver && (
        <Alert className="mt-4">
          ...
          <AlertDescription>
            {guesses.find(guess => guess === solution) ? 'Congratulations! You won!' : `The word was: ${solution}`}
          </AlertDescription>
        </Alert>
      )}
    </div>
